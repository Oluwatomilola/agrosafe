...
contract AgroSafe is Ownable {
    ...
    function registerFarmer(
        ...
        require(
            bytes(name).length >= 2 && bytes(name).length <= 50,
            "Name must be between 2 and 50 characters"
        );
        require(
            bytes(location).length >= 2 && bytes(location).length <= 100,
    ...
    function recordProduce(
        ...
        require(
            bytes(cropType).length >= 2 && bytes(cropType).length <= 50,
            "Crop type must be between 2 and 50 characters"
        );
        require(
